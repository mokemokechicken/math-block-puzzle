...
- DOM 上の残り時間表示は秒が切り替わる境界だけ `setTimeout` で更新する。
- 残り時間ゲージは `transform: scaleX()` で更新し、`width` 更新によるレイアウト再計算を避ける。
- 同じ表示値・style 値の DOM 書き込みは skip する。
- ページが非表示になったら timer UI の更新を止め、再表示時に実時刻から残り時間を再計算して再開する。
